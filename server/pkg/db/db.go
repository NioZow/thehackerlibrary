...
func Setup(reset bool) error {
    ...
    _, err = DB.Exec("CREATE TABLE IF NOT EXISTS resources (id INT PRIMARY KEY NOT NULL AUTO_INCREMENT, type VARCHAR(10), name VARCHAR(100) UNIQUE, date VARCHAR(11), url VARCHAR(150), price SMALLINT, difficulty TINYINT, Time SMALLINT)")
    if err != nil {
        return err
    }
